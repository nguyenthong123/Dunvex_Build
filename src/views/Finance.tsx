...
const Finance = () => {
	...
	const handleAutoGenerateBankTransactions = async (isManual = true) => {
		...
		for (const loan of loans) {
			...
			for (let i = 1; i <= termMonths; i++) {
				...
				const isDuplicateInterest = cashLogs.some(l =>
					l.type === 'chi' &&
					l.category === 'Lãi suất ngân hàng' &&
					l.date === dateStr &&
					(l.parentId === loan.id || (l.note && l.note.includes(`Lãi vay tháng ${i}/${termMonths}`)))
				);
				...
				if (!isDuplicateInterest) {
					...
					batch.set(interestRef, {
						...
						date: dateStr,
						note: `[Tự động] Lãi vay tháng ${i}/${termMonths} - ${loan.bankName || 'Khoản vay'} (Gốc: ${formatPrice(loan.amount)})`,
						parentId: loan.id,
				...
				// Nếu là tháng cuối cùng và đã quá hạn
				if (i === termMonths) {
					const isDuplicatePrincipal = cashLogs.some(l =>
						l.type === 'chi' &&
						l.category === 'Nợ gốc ngân hàng' &&
						l.date === dateStr &&
						(l.parentId === loan.id || (l.note && l.note.includes("Tất toán nợ gốc")))
					);
					...
					if (!isDuplicatePrincipal) {
						...
						batch.set(principalRef, {
							...
							amount: loan.amount,
							date: dateStr,
							note: `[Tự động] Tất toán nợ gốc - ${loan.bankName || 'Khoản vay'} (Hợp đồng ngày ${loan.date})`,
							parentId: loan.id,
							createdAt: serverTimestamp(),
							createdBy: currentUserId,
							isAutoGenerated: true
		...
		if (count > 0) {
			try {
				await batch.commit();
				showToast(`Đã tự động tạo ${count} chứng từ lãi/gốc ngân hàng`, "success");
			} catch (error) {
				console.error("Batch error:", error);
				showToast("Lỗi khi ghi dữ liệu tự động", "error");
			}
		} else {
			showToast("Dữ liệu đã được cập nhật đầy đủ, không có chứng từ mới cần tạo", "info");
		}
	};
