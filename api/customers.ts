...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (req.method === "POST") {
      ...
      if (email) {
        ...
        if (existingDocs.length > 0) {
          ...
          if (!patchRes.ok) {
            ...
            throw new Error(`Failed to update existing customer: ${patchRes.status} ${errText}`);
          }
      ...
      const url = `${FIRESTORE_BASE}/customers`;
      const docBody = {
        fields: {
          name: { stringValue: name },
          phone: { stringValue: phone || "" },
      ...
      if (!createRes.ok) {
        ...
        throw new Error(`Firestore document creation failed: ${createRes.status} ${errText}`);
      }

      const createdData = await createRes.json();
      const newIdParts = createdData.name.split("/");
      const newId = newIdParts[newIdParts.length - 1];
