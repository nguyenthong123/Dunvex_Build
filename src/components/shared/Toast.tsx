		...
		<ToastContext.Provider value={{ showToast, showConfirm }}>
			...
			{confirmDialog && (
				<div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
					<div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-6 max-w-sm w-full mx-4 shadow-2xl animate-in zoom-in-95 duration-200 text-left transition-colors duration-300">
						<div className="flex items-center gap-3 mb-4 text-[#FF6D00]">
							<span className="material-symbols-outlined text-2xl font-bold">help</span>
							<h3 className="text-[#1A237E] dark:text-indigo-400 font-black text-base uppercase tracking-tight">{confirmDialog.title}</h3>
						</div>
						<p className="text-slate-600 dark:text-slate-350 text-xs font-semibold leading-relaxed mb-6">{confirmDialog.message}</p>
						<div className="flex justify-end gap-3">
							<button
								onClick={() => setConfirmDialog(null)}
								className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl text-xs font-black uppercase tracking-wider transition-colors"
							>
								Hủy
							</button>
							<button
								...
								className="px-5 py-2 bg-[#FF6D00] hover:bg-orange-600 text-white rounded-xl text-xs font-black uppercase tracking-wider transition-colors shadow-lg shadow-orange-500/10 active:scale-95"
							>
								Đồng ý
							</button>
						</div>
