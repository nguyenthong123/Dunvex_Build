...
const ProductList = () => {
	...
	return (
		<div className="flex flex-col h-screen bg-[#F8FAFC]">
			{/* MOBILE NAV BOTTOM */}
			<div className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-gray-200 z-50 flex justify-around items-center px-2 pb-safe">
				<ProdNavButton icon="inventory_2" label="Sản phẩm" active={true} />
				<ProdNavButton icon="shopping_cart" label="Đơn hàng" active={false} onClick={() => navigate('/orders')} />
				<div className="relative -top-5">
					<button
						onClick={() => setShowAddForm(true)}
						className="size-14 bg-[#FF6D00] rounded-full shadow-lg shadow-orange-500/40 flex items-center justify-center text-white active:scale-95 transition-transform"
					>
						<span className="material-symbols-outlined text-3xl">add</span>
					</button>
				</div>
				<ProdNavButton icon="people" label="Khách hàng" active={false} onClick={() => navigate('/customers')} />
				<ProdNavButton icon="bar_chart" label="Báo cáo" active={false} onClick={() => navigate('/reports')} />
			</div>

			<div className="flex flex-1 overflow-hidden">
				{/* SIDEBAR (Desktop) */}
				<aside className="hidden lg:flex w-64 flex-col bg-[#1A237E] text-white">
					<div className="p-6 flex items-center gap-3">
						<div className="size-10 bg-white/10 rounded-xl flex items-center justify-center">
							<span className="material-symbols-outlined">apartment</span>
						</div>
						<h1 className="font-black text-xl tracking-tight">DUNVEX</h1>
					</div>

					<nav className="flex-1 px-4 space-y-2 py-4">
						<div
							onClick={() => navigate('/')}
							className="hover:bg-white/5 rounded-xl p-3 flex items-center gap-3 font-medium text-slate-300 cursor-pointer transition-colors"
						>
							<span className="material-symbols-outlined">dashboard</span>
							<span>Command Center</span>
						</div>
						<div className="bg-white/10 rounded-xl p-3 flex items-center gap-3 font-bold cursor-pointer transition-colors text-white">
						...
						<div className="hover:bg-white/5 rounded-xl p-3 flex items-center gap-3 font-medium text-slate-300 cursor-pointer transition-colors">
					...
					<div className="p-4 border-t border-white/10">
						<div className="flex items-center gap-3">
							<div className="size-10 rounded-full bg-gradient-to-tr from-orange-400 to-pink-500 text-xs font-bold flex items-center justify-center">
								{auth.currentUser?.email?.slice(0, 2).toUpperCase()}
							</div>
							<div className="overflow-hidden">
								<p className="font-bold text-sm truncate">{auth.currentUser?.displayName}</p>
								<p className="text-xs text-slate-400 truncate">{auth.currentUser?.email}</p>
							</div>
							<button onClick={() => auth.signOut().then(() => navigate('/login'))} className="ml-auto p-2 hover:bg-white/10 rounded-full">
				...
				<main className="flex-1 flex flex-col min-w-0">
					...
					<header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 md:px-8 shrink-0">
						<div className="flex items-center gap-2">
							<button
								onClick={() => navigate('/')}
								className="lg:hidden p-2 hover:bg-slate-100 rounded-full text-[#1A237E] transition-all"
							>
								<span className="material-symbols-outlined">arrow_back</span>
							</button>
							<h2 className="text-lg md:text-xl font-black text-[#1A237E]">Kho Hàng</h2>
						</div>
...
const ProdNavButton = ({ icon, label, active, onClick }: any) => (
	<button onClick={onClick} className={`flex flex-col items-center gap-1 ${active ? 'text-[#1A237E]' : 'text-gray-400'}`}>
