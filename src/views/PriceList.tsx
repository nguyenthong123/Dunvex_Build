...
const PriceList = () => {
	...
	return (
		<div className="flex flex-col min-h-screen bg-slate-100 dark:bg-slate-950 transition-colors duration-300 print:bg-white print:p-0">
			...
			<header className="h-20 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 md:px-8 shrink-0 sticky top-0 z-40 print:hidden transition-colors">
				<div className="flex items-center gap-4">
					<button
						onClick={() => viewMode === 'detail' ? setViewMode('list') : navigate('/')}
						className="size-10 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-all shadow-sm"
					>
						<ArrowLeft size={20} />
					</button>
				...
				<div className="flex items-center gap-3">
					...
					{viewMode === 'detail' && selectedList?.isUnsaved && (
						<button
							onClick={() => savePriceList(priceData, headers)}
							className="flex items-center gap-2 bg-indigo-600 text-white px-6 py-2.5 rounded-xl font-black text-sm shadow-lg shadow-blue-500/20 hover:bg-indigo-700 transition-all"
						>
							<Save size={18} />
			...
			{/* PAGE CONTENT */}
			<main className={`flex-1 ${viewMode === 'detail' ? 'p-2 md:p-8' : 'p-4 md:p-12'} print:p-0 overflow-x-hidden`}>
				{viewMode === 'list' ? (
					...
					<div className="relative min-h-[800px] flex justify-center">
						...
						<div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[60] bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-full p-1.5 flex items-center gap-1 shadow-[0_20px_50px_rgba(0,0,0,0.5)] print:hidden scale-90 sm:scale-100">
