...
const botResponseSchema: Schema = {
    ...
    properties: {
        intent: {
            ...
            description: "Mục đích của câu lệnh: CREATE_ORDER, UPDATE_ORDER, REVIEW_ORDER, CREATE_CUSTOMER, CREATE_PRODUCT, SEARCH_CUSTOMER, UPDATE_CUSTOMER, CREATE_PAYMENT, INVENTORY_ACTION, UNKNOWN",
        },
        order_id: {
            type: SchemaType.STRING,
...
- Nếu người dùng yêu cầu THÊM SẢN PHẨM VÀO ĐƠN CŨ hoặc SỬA ĐƠN CŨ bằng ID (VD: "thêm sắt hộp vào đơn 6UTWIUJZ", "sửa đơn ID ABC thêm 2 tấm nhựa", "bổ sung xi măng vào đơn hàng XYZ"): Gán intent là UPDATE_ORDER. Trích xuất 'order_id' (ID đơn hàng, VD: 6UTWIUJZ). Trích xuất danh sách sản phẩm cần THÊM vào mảng 'products' (tên, số lượng, category). Nếu thiếu ID đơn hàng, thêm "ID đơn hàng" vào 'missing_info'. LƯU Ý: Hành động này là THÊM sản phẩm, KHÔNG phải thay thế toàn bộ đơn.
