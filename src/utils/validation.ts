...
export const CustomerSchema = z.object({
	...
	lat: z.number().nullish().transform(val => val ?? null),
	lng: z.number().nullish().transform(val => val ?? null),
	taxName: z.string().max(200, "Tên xuất hóa đơn quá dài").optional().transform(val => val?.trim() || ''),
